...
def main():
    ...
    language = tokenizer(args.text, return_tensors="pt")
    timestamps = torch.tensor([[time.time(), time.time(), time.time()]], dtype=torch.float64)
    print(f"device={args.device}")
    print(f"image={tuple(image_tensor.shape)} {image_tensor.dtype}")
    print(f"state={tuple(state.shape)} {state.dtype}")
    print(f"input_ids={tuple(language['input_ids'].shape)} attention_mask={tuple(language['attention_mask'].shape)}")
    print(f"timestamps={tuple(timestamps.shape)} max_skew_ms={(timestamps.max()-timestamps.min())*1000:.3f}")
